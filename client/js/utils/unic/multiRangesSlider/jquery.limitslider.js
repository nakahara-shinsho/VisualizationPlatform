...
;(function ($, undefined) {
	...
	$.widget('vanderlee.limitslider', $.ui.slider, {
		...
		_renderRanges: function() {
          ...
          //add by xinxiao li
          if (range.color) {
						sliderRange.css('background-color', range.color );
					}
          //add by xinxiao li
          if (range.text) {
						sliderRange.text(range.text);
					}
          if(range.background) {
            sliderRange.css('background',  'linear-gradient( to right, '+ range.background[0]+ ','+range.background[1] +')');
          }
          if (left == 0) {
