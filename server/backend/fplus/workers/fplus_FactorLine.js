function MyClass (dbs) {
  ...
  this.asyn = function(options, dataPath) {
    console.log(">> [WORKER] :: fplus_FactorLine.js");
   ...
   // var db = new sqlite3.Database(dataPath+"/fplus.db");
    db.serialize(function(){
      // Generate Command
      var command = "select line,sum(未検出／異常) AS 未検出／異常,sum(検出／異常) AS 検出／異常,sum(未検出／正常) AS 未検出／正常,sum(検出／正常) AS 検出／正常 from errorTable ";
      ...
      db.all(command, function(err, data){
        if(err){
          ...
          for(var i=0; i < data.length; i++){
            ...
            if(row[0] !== -1){
              if((beforeLine + 1) !== row[0]){
                if(beforeLine !== -1){
                  csv.push((beforeLine + 1) + ",0,0,0,0");
                }
                beforeLine = row[0];
              }
              csv.push(row.join(","));
